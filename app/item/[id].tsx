...
export default function ItemDetailScreen() {
  const { id, storeId, storeName, storeAddress } =
    useLocalSearchParams<{ id: string; storeId?: string; storeName?: string; storeAddress?: string }>();
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [itemData, reportData] = await Promise.all([
        ...
        fetchRecentReports(id as string, storeId ?? null),
      ]);
      setItem(itemData);
          ...
          <>
            ...
            <View style={styles.itemCard}>
              <View style={styles.itemIconBox}>
                <Ionicons name="basket-outline" size={26} color={PRIMARY} />
              </View>
              <View style={styles.itemInfo}>
                <Text style={styles.itemName}>{item.name}</Text>
                <Text style={styles.itemSub}>
                  {subtitle ? `${subtitle} · ${item.category}` : item.category}
                </Text>
              </View>
            ...
            {withPhotos.length > 0 && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>
                  Shelf Photos{' '}
                  <Text style={styles.sectionCount}>({withPhotos.length})</Text>
                </Text>
                <FlatList
                  horizontal
                  data={withPhotos}
                  ...
                  renderItem={({ item: r }) => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.85}
                    >
                      <Image source={{ uri: r.photo_url! }} style={styles.thumbnail} />
...
function ReportRow({ report, onPhotoPress }: { report: RecentReport; onPhotoPress: () => void }) {
...
const styles = StyleSheet.create({
  ...
  itemIconBox: {
    width: 46, height: 46, borderRadius: 12, backgroundColor: '#ECFDF5',
    alignItems: 'center', justifyContent: 'center',
  },
  itemInfo:    { flex: 1 },
  itemName:    { fontSize: 16, fontWeight: '700', color: '#111827', marginBottom: 2 },
  itemSub:     { fontSize: 12, color: '#9CA3AF', fontWeight: '500' },
  ...
  thumbnail: {
    width: 110, height: 80, borderRadius: 10, backgroundColor: '#E5E7EB',
  },
  thumbOverlay: {
  ...
  reportThumb: {
    width: 52, height: 40, borderRadius: 8, backgroundColor: '#E5E7EB',
  },
  reportThumbBadge: {
  ...
  lightboxImage: { width: SCREEN_WIDTH, height: SCREEN_WIDTH * 0.85 },
  lightboxClose: {
